/// @description ACTIVATE
/// @param {bool} true or false
function ConfigAndroid() 
{
	if argument0 = true
	{
		oAndroidButton1.CONFIG = true
		oAndroidButton2.CONFIG = true
		oAndroidButtonLeft.CONFIG = true
		oAndroidButtonRight.CONFIG = true
		oAndroidButtonUp.CONFIG = true
		oAndroidButtonDown.CONFIG = true
	}
	else
	{
		oAndroidButton1.CONFIG = false
		oAndroidButton2.CONFIG = false
		oAndroidButtonLeft.CONFIG = false
		oAndroidButtonRight.CONFIG = false
		oAndroidButtonUp.CONFIG = false
		oAndroidButtonDown.CONFIG = false
	}
	
	
}
